...
<!--                    สำหรับ A4 
<div style="width:78mm; padding-left:10px; padding-right:10px; padding-top:0px; margin-left:auto; margin-right:auto;">
...
<div style="width:78mm; padding:2px; margin-left:auto; margin-right:auto;">
...
<?php endforeach; ?>
<?php endif; ?>
<center><img src="<?php echo base_url()."assets/barcode/barcode.php?text=".$rs->reference; ?>" style='width:80%;' /></center>
<hr style='border-color:#CCC; margin-top: 10px; margin-bottom:15px;' />
<center>วันที่ : <?php echo thaiDate($rs->date_add); ?></center>
<center>ต้นทาง : <?php echo getShopName($rs->id_shop); ?> &nbsp;&nbsp;ปลายทาง : <?php echo getShopName($rs->id_target); ?></center>
<center>ทะเบียนรถ : <?php echo getCarPlate($rs->id_car); ?>  </center>
<center>พนักงานขับรถ : <?php echo getDriverName($rs->id_driver); ?></center>
<hr style='border-color:#CCC; margin-top: 10px; margin-bottom:15px;' />
<?php if($detail != false) : ?>
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="font-size:10px;">
 <tr><td width="70%">รหัสอ้างอิง</td><td width="20%">ลูกค้า</td><td width="10%" align="right">Qty</td></tr>
 <?php $total_qty = 0; ?>
...
?>
	<tr>
    <td style="padding-bottom:5px;"><img src="<?php echo base_url()."assets/barcode/barcode.php?text=".$order_no; ?>" style='width:80%;' /></td>
    <td><?php echo $customer_name; ?></td>
    <td align="right"><?php echo number_format($order_qty); ?></td></tr>
  <?php $total_qty += $order_qty; ?>
<?php endforeach; ?>
	<tr><td colspan="2" align="right">+++ รวม +++</td><td align="right"><?php echo number_format($total_qty); ?></td></tr>
</table>
<hr style='border-color:#CCC; margin-top: 10px; margin-bottom:15px;' />
<div id='seller_copy'>
    <center>&nbsp;</center>
...
<?php 
		$customer_name 	= getCustomerName($or->id_customer);
		$order_no 			=  $or->order_no;
		$order_qty			= get_delivery_qty_by_order($or->id_delivery, $or->id_order);
		$order_amount 	= get_delivery_amount_by_order($or->id_delivery, $or->id_order);
		$order_weight		= get_delivery_weight_by_order($or->id_delivery, $or->id_order);
