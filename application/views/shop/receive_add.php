...
<div class='row'>
	<div class='col-lg-6'>
    	<h3 style='margin-bottom:0px; margin-top:0px;'><i class='fa fa-truck'></i>&nbsp; รับคืนสินค้า</h3>
    </div>
    <div class="col-lg-6">
    	<p class='pull-right'>
        <?php if( isset($detail) ) : ?>
        	<a href="<?php echo base_url(); ?>shop/receive/add/<?php echo $id_delivery; ?>">
        		<button class='btn btn-warning'><i class='fa fa-arrow-left'></i>&nbsp; กลับ</button>
             </a>
         <?php else : ?>
         	<a href="<?php echo base_url(); ?>shop/receive/">
        		<button class='btn btn-warning'><i class='fa fa-arrow-left'></i>&nbsp; กลับ</button>
             </a>
         <?php endif; ?>   
         </p>
...
<div class='row'>
<?php echo form_open(base_url()."shop/receive/receive_add/".$id_delivery); ?>
...
<?php if(isset($detail) && $detail != false) : ?>
    <?php echo form_open(base_url()."shop/receive/receive_detail", array("id"=>"form_receive_detail")); ?>
    <input type="hidden" name="id_order" id="id_order" value="<?php echo $id_order; ?>"  />
    <input type="hidden" id="id_delivery" value="<?php echo $id_delivery; ?>" />
<table class="table table-striped">
<thead>
<tr>
 <th style="width:5%; text-align:center">ลำดับ</th><th style="width:60%;">รายการ</th><th style="width:10%; text-align:center">ส่งมา</th><th style="width:10%; text-align:center">รับเข้า</th><th>&nbsp;</th> 
 </tr>
 </thead>
<?php $n = 1; ?>        
<?php foreach($detail as $rs ) : ?>
    ...
    <td align="center"><?php echo number($rs->qty); ?></td><input type="hidden" id="qty_<?php echo $rs->id_order_detail; ?>" value="<?php echo $rs->qty; ?>" />
    <td align="center"><input type="text" id="txt_<?php echo $rs->id_order_detail; ?>" class="form-control" value="<?php echo $rs->qty; ?>" /></td>
    <td align="right">
    <button type="button" id="btn_<?php echo $rs->id_order_detail; ?>" class="btn btn-success" onclick="add_to_received(<?php echo $rs->id_order_detail; ?>);">
    	<i class="fa fa-plus"></i>&nbsp; รับเข้า</button></td>
    </tr>
<?php $n++; ?>    
<?php endforeach; ?>  
</table>
...
function add_to_received(id){
	...
	$("#btn_save").click(function(e) {
		...
		var id_delivery = $("#id_delivery").val();
		window.location.href="<?php echo $this->home."/order_received/"; ?>"+id_order+"/"+id_delivery;
    });
