...
function fncSubmit()
{
	...
	if(document.add_delivery.target_id.value == "")
	{
		alert('Select target');
		document.add_delivery.target_id.focus();
		return false;
	}
  ...
  <form name="add_delivery" method="post" action="submit_deliver.php?action=driver" onSubmit="JavaScript:return fncSubmit();">
    <table border="0" width="100%">
    	<tr>
        <td width="5%" align="right">Driver : </td>
        
        	<td width="20%">  <select name="driver_id"  class="form-control" id="driver_id" >
...
<? if($delivery_id != ""){
	...
	?>
    <form method="post" action="submit_deliver.php?action=driver_order" id="submit_order_code" >
        <table border="0" width="100%">
        <tr >
  ...
  <?php
	...
	while($i<$row)
	{
		...
		$content.= "<td>".$no."</td><td>".$order_no."</td><td>".$qty."</td><td>".$weight1."</td><td>".$order_date."</td><td>".$due_date."</td><td>".$condition."</td><td>".$status."</td><td><a href='submit_deliver.php?action=del_order&delivery_id=$delivery_id&delivery_detail_id=$delivery_detail_id&order_id=$order_id'><input type='button' class='btn btn-default btn-xs' value='DEL'/></td></tr>";
		 $i++;
		 $no++;
		}
		if($row < 1 ){
...
<?php
	...
	while($i<$row)
	{
		...
		$status = $result['status_name'];
                ...
                $sum_weight = $sum_weight + $weight; //น้ำหนักรวม
		$delivery_detail_id = $result['delivery_detail_id'];
		if($urgent_id>1){
			$content .= "<tr style='color:red;'>";
		}else{ $content.= "<tr>";}
