...
{		
		...
		$qr_o= dbQuery("select * from tbl_order where order_code = '$order_code' and status = '5'");
		$rs_o = dbFetchArray($qr_o); 
		$order_id = $rs_o['order_id'];
		if($order_id != ""){
		$sql = "UPDATE tbl_order SET status = '6' WHERE order_id = ".$order_id;
		dbQuery($sql);
		$sql = "UPDATE tbl_order_detail SET status = '6' WHERE order_id = ".$order_id;
		dbQuery($sql);
		$sql = "UPDATE tbl_delivery_detail SET status = '6' WHERE status = '5' and order_id = ".$order_id;
		dbQuery($sql);
		$qr_dt= dbQuery("select * from tbl_delivery_detail where order_id = '$order_id' and status = '6'");
		$rs_dt = dbFetchArray($qr_dt); 
		$delivery_id = $rs_dt['delivery_id'];
		$qr_d= dbQuery("select * from tbl_delivery_detail where delivery_id = '$delivery_id' and status = '5'");
		$rs_d = dbFetchArray($qr_d);
		$delivery_detail_id = $rs_d['delivery_detail_id'];
		if($delivery_detail_id == ""){
		$sql = "UPDATE tbl_delivery SET status = '6' WHERE delivery_id = ".$delivery_id;
		}
		dbQuery($sql);
		header("location:index.php?content=$content&car_id=$car_id");
		}else{
...
function del_driver_order()
{
		...
		$sql = "UPDATE tbl_order SET status = '5' WHERE order_id = ".$order_id;
		dbQuery($sql);
		$sql = "UPDATE tbl_order_detail SET status = '5' WHERE order_id = ".$order_id;
		dbQuery($sql);
		$sql = "UPDATE tbl_delivery_detail SET status = '5' WHERE status = '6' and order_id = ".$order_id;
		dbQuery($sql);
		$qr_dt= dbQuery("select * from tbl_delivery_detail where order_id = '$order_id' and status = '5'");
		$rs_dt = dbFetchArray($qr_dt); 
		$delivery_id = $rs_dt['delivery_id'];
		$qr_d= dbQuery("select * from tbl_delivery_detail where delivery_id = '$delivery_id' and status = '5'");
		$rs_d = dbFetchArray($qr_d);
		$delivery_detail_id = $rs_d['delivery_detail_id'];
		if($delivery_detail_id != ""){
		$sql = "UPDATE tbl_delivery SET status = '5' WHERE delivery_id = ".$delivery_id;
		}
		dbQuery($sql);
		header("location:index.php?content=$content&car_id=$car_id");
	}
