<h3><? echo $pageTitle;?></h3><?
		...
		if($content == "deliveryhub"){$status = "13";$shipping = "1";}
		else if($content == "deliveryshop"){$status = "15";$shipping = "1";}
		else if($content == "deliverycus"){$status = "10";$shipping = "4";}
		else if($content == "deliverypost"){$status = "11";$shipping = "2";}
		else if($content == "deliveryems"){$status = "12";$shipping = "3";}
...
<td width="3%"></td>
      ...
      <table border="0" width="100%">
      <tr>
    	  <td align="right" width="20%">DRIVER NAME : </td>
    	  <td width="20%"><h4><input type="text" name="" value="<?=$driver_name;?>" class="input-label" disabled="disabled"  /></h4></td>
    	  <td width="15%">CAR PLATE : </td>
    	  <td width="15%"><h4><input type="text" name="" value="<?=$car_plate;?>" class="input-label" disabled="disabled"  /></h4></td>
    	  <td width="15%">TARGET : </td>
    	  <td width="15%"><h4><input type="text" name="" value="<?=$target_name;?>" class="input-label" disabled="disabled"  /></h4></td>
...
<? if($delivery_id != ""){
    ...
    <form method="post" action="submit_deliver.php?action=driver_order&content=<?=$content;?>" id="submit_order_code" >
        ...
        <td width="2%"><input type="hidden" name="delivery_id" id="delivery_id" value="<?=$delivery_id;?>" /><input type="hidden" name="content" value="<?=$content;?>"  /><input type="hidden" name="status" value="<?=$status;?>"  /> <input type="hidden" name="shipping_id" value="<?=$shipping;?>" /></td>
        <td width="10%"><input type="submit" class="form-control" value="Add" /></td>
        <td width="10"></td>
        <td width="33%"><? if($not_order != "" ){echo "<div class='alert h4 alert-danger'><center>$not_order</center></div>";}?></td>
        <td width="10%">
<input name="button" id="button" class="form-control" value="Print" onClick="print();" type="button"   />
</td>
        </tr>
        </table>
        </form>
  ...
  <?php
	...
	$sql ="SELECT * FROM tbl_order LEFT JOIN tbl_status ON tbl_order.status = tbl_status.status_id  LEFT JOIN tbl_urgent ON tbl_order.urgent_id = tbl_urgent.urgent_id LEFT JOIN tbl_delivery_detail ON tbl_order.order_id = tbl_delivery_detail.order_id WHERE tbl_order.status = '$status' and delivery_id = '$delivery_id' ORDER BY tbl_order.urgent_id DESC";
	$query=dbQuery($sql);
	$row = dbNumRows($query);
		$i=0;
	$no = 1;
...
?>
	...
	$sql ="SELECT * FROM tbl_order LEFT JOIN tbl_status ON tbl_order.status = tbl_status.status_id  LEFT JOIN tbl_urgent ON tbl_order.urgent_id = tbl_urgent.urgent_id WHERE tbl_order.status = '9' and shipping_id IN ($shipping) ORDER BY tbl_order.urgent_id DESC";
	$query=dbQuery($sql);
	$row = dbNumRows($query);
	$i=0;
	$no = 1;
