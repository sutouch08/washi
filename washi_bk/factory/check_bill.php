...
var deposit=document.checkbill.deposit.checked;
	//alert('Y' + money);
	if(deposit == false){
    	...
    	<div class="col-md-6">
				...
				?>                       
                  ...
                  <?
					...
					?>
                     <tr>
                        <td colspan="3" rowspan="5">( <? echo bahtThai($sum_discount1);?> )</td><td width="15%" ><?=$sum_qty;?></td><td width="15%" align="right"><?=$sum_amount;?></td>
                    </tr>
                     <tr>
                    ...
                    <tr>
                     ...
                     <tr>
                       <td width="15%">ยอดคงเหลือ</td><td width="15%" align="right"><? echo $sum_discount1;?><input type="hidden" id="sum_discount1" value="<?=$sum_discount1;?>" /><input type="hidden" id="sum_qty" value="<?=$sum_qty;?>"  /></td>
                    </tr>
                </table>
                </td>
                </tr>
...
<tr>
<td width="70%">
<form method="post" name="checkbill" action="submit_checkbill.php"  onSubmit="return bill()" ><div class="col-md-7"><input type="text" name="money" id="money" class="form-control"  placeholder="ชำระเงินสด" required autofocus ><input type="hidden" name="order_id" value="<?=$order_id;?>"  ></div><div class="col-md-2"><input name="deposit" type="checkbox" id="deposit" value="1" /> มัดจำ</div><div class="col-md-3"> <input type="submit" value="ชำระเงินสด" class="form-control" ></div></form></td> 
</tr>
 							<?php  
							$qr_m =dbQuery("SELECT * FROM tbl_member_detail LEFT JOIN tbl_member ON tbl_member_detail.mem_id = tbl_member.mem_id LEFT JOIN tbl_promo ON tbl_member_detail.promo_id = tbl_promo.promo_id LEFT JOIN tbl_credit_type ON tbl_member_detail.credit_type = tbl_credit_type.credit_type where tbl_member_detail.mem_id = '$mem_id' and tbl_member_detail.credit_type != '$credit_type' and ((detail_start <= '$date' and detail_end >= '$date') or detail_start = '0000-00-00') ORDER BY tbl_promo.promo_id ASC");	
							...
							?>
<tr height="50">
<td width="70%"><? if($credit_type_mem == "$credit_type"){?><div class="col-md-8"><input type="button" class="form-control" value="<?=$rs_m['promo_name'];?>" disabled="disabled" ></div><? }else{?><a href="submit_checkbill.php?mem_detail_id=<?=$mem_detail_id;?>&order_id=<?=$order_id;?>&mem_id=<?=$mem_id;?>" onclick="return check('<?=$mem_detail_id;?>')" ><div class="col-md-8"><input type="button" class="form-control"  value="<?=$rs_m['promo_name'];?>"></div></a><? }?><div class="col-md-4" align="left">คงเหลือ <?=$rs_m['detail_credit'];?> <?=$rs_m['credit_unit'];?><input type="hidden" id="detail_credit<?=$mem_detail_id;?>" value="<?=$rs_m['detail_credit'];?>" /><input type="hidden" id="credit_type_mem<?=$mem_detail_id;?>" value="<?=$credit_type_mem;?>" /></div></td>
</tr>
<? $i++;
	}
	?>
