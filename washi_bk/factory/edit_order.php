 <div class="container">
    ...
    <tr ><td width="5%"><h4>#</h4></td><td width="20%"><h4>Order No.</h4></td><td width="10%"><h4>Qty.</h4></td><td width="10%"><h4>Amount</h4></td><td width="20%"><h4>Customer</h4></td><td width="15%"><h4>Order Date</h4></td><td width="10%"><h4>Due Date</h4></td><td width="10%"><h4>Condition</h4></td></tr></thead>
	  ...
	  $shop_id=$_COOKIE['shop_id'];
	    $sql = "SELECT * FROM tbl_order LEFT JOIN tbl_customer ON tbl_order.customer_id = tbl_customer.customer_id  LEFT JOIN tbl_urgent ON tbl_order.urgent_id = tbl_urgent.urgent_id where tbl_order.shop_id = '$shop_id' and tbl_order.status IN (1,6)  ORDER BY tbl_order.urgent_id DESC";
		$result = dbQuery($sql);
