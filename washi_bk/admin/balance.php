...
{		
		echo "<option "; if($id=="เลือกร้าน"){echo "selected='selected'";} echo ">เลือกร้าน</option>";
		$sql = "select shop_id, shop_name from tbl_shop where shop_id != '2'";
		$result = dbQuery($sql);
 ...
 <div class="container">
    ...
    <form method="post">
    <table border="0" width="100%"><tr><td width="10%">Shop</td><td width="15%"><select class="form-control" name="shop_id"><? shop_list($shop_id); ?></select></td><td width="15%">Select  duration from :</td><td width="15%"><input type="text" class="form-control"  name="from_date" id="from_date"   value="<?  if(isset($_POST['from_date'])&& $_POST['to_date']){ echo date('d-m-Y',strtotime($from_date));} else { echo date('d-m-Y',strtotime($today));} ?>"/></td>
...
if(isset($_POST['from_date'])&& $_POST['to_date']){
		  ...
		  if($shop_id == "เลือกร้าน"){
				 $sql ="SELECT tbl_order.order_id,order_no,tbl_pament.order_amount,pament_deposit,promo_id,promo_amount,payment_time,paymet_received,payment_change FROM tbl_pament LEFT JOIN tbl_order ON tbl_pament.order_id = tbl_order.order_id  WHERE payment_time BETWEEN  '$from_date' AND '$to_date' ORDER BY tbl_pament.payment_id DESC";
			}else{
	...
	$sql ="SELECT tbl_order.order_id,order_no,tbl_pament.order_amount,pament_deposit,promo_id,promo_amount,payment_time,paymet_received,payment_change FROM  tbl_pament LEFT JOIN tbl_order ON tbl_pament.order_id = tbl_order.order_id  WHERE  payment_time LIKE '%$today%' ORDER BY tbl_pament.payment_id DESC";
}
	$query=dbQuery($sql);
	$row = dbNumRows($query);
	$i=0;
