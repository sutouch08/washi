    ...
    <table border="0" width="100%">
        ...
        <td width="5%" align="right">Driver : </td>
            ...
            <input type="hidden" value="<?=$status_id;?>" name="status_id" />
             <input type="hidden" name="delivery_id" value="<?=$delivery_id;?>"  /><select name="driver_id"  class="form-control" id="driver_id" >
...
<td width="10%" align="left"> <?=$target_name;?>
</td>
<td width="3%"></td>
        	<td width="5%"><input type="submit" class="form-control"  value="Save"></td>
            <td width="32%"> <? if($Y == "Y"){ echo "<img src=\"../images/Approve_icon.jpg\" width=\"30\" height=\"30\" />";}?></td>
        </tr>
        </table>  </form>
        ...
        <td width="2%"><input type="hidden" name="delivery_id" id="delivery_id" value="<?=$delivery_id;?>" />
        <input type="hidden" name="status_id"  value="<?=$status_id;?>"  /></td>
  ...
  <?php
	...
	while($i<$row)
	{
		...
		if($urgent_id>1){$content1.= "<tr style='color:red;'>";}else{ $content1.= "<tr>";}
				$content1.= "<td>".$no."</td><td>".$order_no."</td><td>".$qty."</td><td>".$weight1."</td><td>".$order_date."</td><td>".$due_date."</td><td>".$condition."</td><td>".$status_name."</td><td><a href='submit_edit.php?action=del_order&delivery_id=$delivery_id&delivery_detail_id=$delivery_detail_id&order_id=$order_id&status_id=$status_id'><input type='button' class='btn btn-default btn-xs' value='DEL'/></td></tr>";
		 $i++;
		 $no++;
		}
		if($row < 1 ){
...
<?php
	...
	$sql ="SELECT * FROM tbl_order LEFT JOIN tbl_status ON tbl_order.status = tbl_status.status_id  LEFT JOIN tbl_urgent ON tbl_order.urgent_id = tbl_urgent.urgent_id WHERE tbl_order.status = '$status_id'-1  ORDER BY tbl_order.urgent_id DESC";
	$query=dbQuery($sql);
	$row = dbNumRows($query);
	$i=0;
	$no = 1;
	...
	while($i<$row)
	{
		...
		if($urgent_id>1){$content1.= "<tr style='color:red;'>";}else{ $content1.= "<tr>";}
				$content1.= "<td>".$no."</td><td>".$order_no."</td><td>".$qty."</td><td>".$weight."</td><td>".$order_date."</td><td>".$due_date."</td><td>".$condition."</td><td>".$status_name."</td><td></td></tr>";
		 $i++;
		 $no++;
		}
		if($row < 1 ){
