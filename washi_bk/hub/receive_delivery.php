<?
		...
		if($content == "Receive_factory"){$status = "13";}
  ...
  <?php
	...
	while($i<$row)
	{
		...
		if($urgent_id>1){$content1.= "<tr style='color:red;'>";}else{ $content1.= "<tr>";}
				$content1.= "<td>".$no."</td><td>".$order_no."".more($complete)."</td><td>".$qty."</td><td>".$weight."</td><td>".$order_date."</td><td>".$due_date."</td><td>".$shop_name."</td><td>".$status_name."</td><td><a href='submit_receive.php?action=del_order&order_id=$order_id&content=$content&car_id=$car_id&status=$status'><input type='button' class='btn btn-default btn-xs' value='DEL'/></a></td></tr>";
		 $i++;
		 $no++;
		}
		if($row < 1 ){
...
?>
     ...
     <thead>
          <tr><td colspan="4"><h4>HUB</h4></td><td align="right"><h4>จำนวน ORDER : </h4></td><td align="left"><h4><?php echo $row;?> ORDER</h4></td><td align="right"><h4>หนัก : </h4></td><td align="left"><h4><?php echo $sum_weight;?> กก.</h4></td></tr>
	...
	while($i<$row)
	{
		...
		if($urgent_id>1){$content.= "<tr style='color:red;'>";}else{ $content.= "<tr>";}
				$content.= "<td>".$no."</td><td>".$order_no."".more($complete)."</td><td>".$qty."</td><td>".$weight."</td><td>".$order_date."</td><td>".$due_date."</td><td>".$shop_name."</td><td>".$status_name."</td><td></td></tr>";
		 $i++;
		 $no++;
		}
		if($row < 1 ){
