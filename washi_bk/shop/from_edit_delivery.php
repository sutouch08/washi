...
<?php  
  ...
  <form name="edit_delivery" method="post" action="submit_edit.php?action=driver">
    <table border="0" width="100%">
    	<tr>
        <td width="5%" align="right">Driver : </td>
        
        	<td width="20%">  <input type="hidden" name="delivery_id" value="<?=$delivery_id;?>"  /><select name="driver_id"  class="form-control" id="driver_id" >
    ...
    ?>        
            ...
            <option value="<?=$car_id;?>"><?=$car_plate;?></option>
            <?
    ...
    <td width="5%"  align="right">Target : 
    </td>
<td width="20%"><select name="target_id" id="target_id" class="form-control">
				<option value="<?=$target_id;?>"><?=$target_name;?></option>
                 <?
    $qr_t= dbQuery("select  * from tbl_target ORDER BY target_id ASC ");
	$row = mysql_num_rows($qr_t);
	$i=0;												
	while($i<$row){
	$rs_t = dbFetchArray($qr_t); 
	$target_id1 = $rs_t['target_id'];
	$target_name1 = $rs_t['target_name'];
	 echo "<option value='$target_id1'>$target_name1</option>";
	 $i++;
	}
    ?>        
    </select>
...
<td width="3%"></td>
        	<td width="5%"><input type="submit" class="form-control"  value="Save"></td>
            <td width="22%"> <? if($Y == "Y"){ echo "<img src=\"../images/Approve_icon.jpg\" width=\"30\" height=\"30\" />";}?></td>
        </tr>
        </table>  </form>
        
          <hr style="margin-top:5px; margin-bottom:5px;" />
           <form method="post" action="submit_edit.php?action=driver_order"  >
        <table border="0" width="100%">
        <tr >
        ...
        <td width="2%"><input type="hidden" name="delivery_id" id="delivery_id" value="<?=$delivery_id;?>" /><input type="hidden" name="status_id" value="<?=$status_id;?>" /></td>
   ...
   <?php
	...
	$sql ="SELECT * FROM tbl_order LEFT JOIN tbl_status ON tbl_order.status = tbl_status.status_id  LEFT JOIN tbl_urgent ON tbl_order.urgent_id = tbl_urgent.urgent_id LEFT JOIN tbl_delivery_detail ON tbl_order.order_id = tbl_delivery_detail.order_id WHERE tbl_order.status = '$status_id' and delivery_id = '$delivery_id' ORDER BY tbl_order.urgent_id DESC";
	$query=dbQuery($sql);
	$row = dbNumRows($query);
	$i=0;
	$no = 1;
	...
	while($i<$row)
	{
		...
		$content.= "<td>".$no."</td><td>".$order_no."</td><td>".$qty."</td><td>".$weight1."</td><td>".$order_date."</td><td>".$due_date."</td><td>".$condition."</td><td>".$status."</td><td><a href='submit_edit.php?action=del_order&delivery_id=$delivery_id&delivery_detail_id=$delivery_detail_id&order_id=$order_id'><input type='button' class='btn btn-default btn-xs' value='DEL'/></td></tr>";
		 $i++;
		 $no++;
		}
		if($row < 1 ){
...
<?php
	...
	$sql ="SELECT * FROM tbl_order LEFT JOIN tbl_status ON tbl_order.status = tbl_status.status_id  LEFT JOIN tbl_urgent ON tbl_order.urgent_id = tbl_urgent.urgent_id LEFT JOIN tbl_delivery_detail ON tbl_order.order_id = tbl_delivery_detail.order_id WHERE tbl_order.status = '2' ORDER BY tbl_order.urgent_id DESC";
	$query=dbQuery($sql);
	$row = dbNumRows($query);
	$i=0;
	$no = 1;
	...
	while($i<$row)
	{
		...
		$urgent_id = $result['urgent_id'];
		$status = $result['status_name'];
                $weight = $result['weight']; //น้ำหนัก
                $sum_weight = $sum_weight + $weight; //น้ำหนักรวมที่จะเอาขึ้นไปโชบนหัวตารางที่บรรทุกได้อะคับ
		$delivery_detail_id = $result['delivery_detail_id'];
		if($urgent_id>1){
			$content .= "<tr style='color:red;'>";
		}else{ $content.= "<tr>";}
		$content.= "<td>".$no."</td><td>".$order_no."</td><td>".$qty."</td><td>".$weight."</td><td>".$order_date."</td><td>".$due_date."</td><td>".$condition."</td><td>".$status."</td><td></td></tr>";
		 $i++;
		 $no++;
		}
		if($row < 1 ){
...
?>
 ...
 <table class="table table-hover">
      <thead>
          <tr><td colspan="4"><h4>SHOP</h4></td><td align="right"><h4>จำนวน ORDER : </h4></td><td align="left"><h4><?php echo $row;?> ORDER</h4></td><td align="right"><h4>หนัก : </h4></td><td align="left"><h4><?php echo $sum_weight;?> กก.</h4></td></tr>
    <tr ><td width="5%">#</td><td width="20%">Order No.</td><td width="5%">Qty.</td><td width="5%">หนัก</td><td width="15%">Order Date</td>
    <td width="15%">Due Date</td><td width="10%">Condition</td><td width="20%">Status</td><td width="5%"></td></tr>	</thead>
<?php echo $content;?>
</table>
